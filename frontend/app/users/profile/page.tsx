...
import { userApi, UserResponse, MyQuestionSummaryResponse, MyAnswerSummaryResponse, MyCommentSummaryResponse } from '@/api/user';
...
export default function ProfilePage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [userData, questionsData, answersData, commentsData] = await Promise.all([
        userApi.getUser(),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 mb-8 animate-fade-in">
        ...
        <div className="mb-6">
          <label className="block text-sm font-semibold text-slate-700 mb-2">사용자 이름</label>
          {editingUsername ? (
            ...
            <div className="flex items-center gap-3">
              <span className="text-lg text-slate-900 font-medium">{user.username}</span>
              <button
        ...
        <div className="grid grid-cols-2 gap-6 pt-6 border-t border-slate-200">
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-4">
            <label className="block text-sm font-semibold text-slate-600 mb-1">포인트</label>
            <div className="text-2xl font-bold text-indigo-600">{user.point}</div>
          </div>
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4">
            <label className="block text-sm font-semibold text-slate-600 mb-1">레벨</label>
            ...
            <div className="space-y-4">
              {myQuestions.length === 0 ? (
                ...
                myQuestions.map((question) => (
                  ...
                  >
                    <h3 className="font-semibold text-slate-900 mb-2 hover:text-indigo-600 transition-colors">{question.title}</h3>
              ...
              {myAnswers.length === 0 ? (
                ...
                myAnswers.map((answer) => (
                  <Link
                    ...
                    className="block p-5 border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-indigo-300 transition-all"
                  >
                    <p className="text-slate-700 mb-2 line-clamp-2">{answer.content}</p>
              ...
              {myComments.length === 0 ? (
                ...
                myComments.map((comment) => (
                  <Link
                    ...
                    href={comment.isQuestion ? `/questions/${comment.postId}` : `#`}
                    className="block p-5 border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-indigo-300 transition-all"
                  >
