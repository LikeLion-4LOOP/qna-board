    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="mb-6 bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="mt-4 pt-4 border-t border-slate-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-slate-700">선택된 카테고리:</span>
                <span className={`inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r ${getCategoryById(selectedCategory).color} text-white rounded-xl text-sm font-semibold`}>
        ...
        {!selectedCategory && allTags.length > 0 && (
          <div className="mt-4 pt-4 border-t border-slate-200">
            <h3 className="text-sm font-semibold text-slate-700 mb-3">태그별 보기</h3>
      ...
      {error && (
        <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-6">
          ...
          <div className="text-sm text-red-600 space-y-1">
            <p>백엔드 서버가 실행 중인지 확인해주세요 (http://3.35.209.3:8080)</p>
            <p>데이터베이스 연결이 정상인지 확인해주세요</p>
            <p>브라우저 개발자 도구의 네트워크 탭에서 상세 에러를 확인해주세요</p>
          </div>
        </div>
      )}
      ...
      {!loading && !error && filteredQuestions.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-16 text-center">
          ...
          <p className="text-slate-500 text-lg">
            {searchQuery || selectedTag ? '검색 결과가 없습니다.' : '등록된 질문이 없습니다.'}
          </p>
          <p className="text-slate-400 text-sm mt-2">
            {searchQuery || selectedTag ? '다른 검색어나 필터를 시도해보세요.' : '첫 번째 질문을 작성해보세요!'}
          </p>
        </div>
      ) : !loading && !error ? (
